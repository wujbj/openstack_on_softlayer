...
def generate_knife_commands(localknifefile, localrolesfile, clusterelement, overwrite_chef_definitions) :
  ...
  appendnodenames = getAppendNodeNames(clusterelement);
  ...
  #nodelist = clusterelement.getElementsByTagName('node')
  nodelist = getAppendNodeLists(clusterelement);
...
localknifefile = "/tmp/"+clustername+"_appendrunall.sh"
...
if chefserver != "localhost" :
  os.system("scp "+localknifefile+" root@"+chefserver+":/tmp/"+clustername+"_appendrunall.sh")
  os.system('ssh root@'+chefserver+' "chmod 755 /tmp/'+clustername+'_appendrunall.sh"')
else :
...
for node in root.findall('cluster'):
