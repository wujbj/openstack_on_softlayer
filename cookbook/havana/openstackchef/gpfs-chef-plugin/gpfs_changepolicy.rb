...
#
## Install 
# Place in .chef/plugins/knife/gpfs_changepolicy.rb
#
## Usage:
# knife gpfs change policy --master-node="devr1n21" --policy-file="/root/gpfs/change_policy_definition" --roles-list="role[gpfs_change_policy]"  --env-file="buddha-253"  --rsa-file="/root/.ssh/id_rsa"  --disktro-option=rhel
###
# add_nodes
...
module MyGpfsChangePolicyNamespace
 class GpfsChangePolicy < Chef::Knife

  banner "knife gpfs change policy (options)"
  ...
  def run
    ...
    if config[:nodeslist_file].nil?
      system " knife gpfs change policy --help "
