...
execute "apply_license_to_these_added_nodes" do
  command "cd /tmp/gpfs_working ; awk -F ':' '{print $1}' /tmp/gpfs_working/add_nodes | grep -v '#' | grep -v grep > /tmp/gpfs_working/add-client-nodes-list ;  /usr/lpp/mmfs/bin/mmchlicense client --accept -N /tmp/gpfs_working/add-client-nodes-list ; cd - "
  ignore_failure true
  action :run
end

execute "start_GPFS_on_these_added_nodes" do
  command "cd /tmp/gpfs_working ; awk -F ':' '{print $1}' /tmp/gpfs_working/add_nodes | grep -v '#' | grep -v grep > /tmp/gpfs_working/add-client-nodes-list ;  /usr/lpp/mmfs/bin/mmstartup -N /tmp/gpfs_working/add-client-nodes-list ; cd - "
  ignore_failure true
  action :run
end
