...
module RCB
  ...
  def get_config_endpoint(server, service, nodeish=nil, partial = false)
    ...
    if svc = rcb_safe_deref(nodeish, "#{server}.services.#{service}")
      retval["network"] = svc["network"] || "nova"
  ...
  def get_bind_endpoint(server, service, nodeish=nil, externalip=nil)
     
    Chef::Log.info("Get bind endpoint server #{server} #{service} #{nodeish} #{externalip}")
    ...
    if not retval.empty?
      ...
      # If externalip=nil or enviroment parameter endpoint="",ignore externalip.
      if externalip.nil? or (externalip == "endpoint" and
           (node["osops_networks"][externalip] == "" or 
           node["osops_networks"][externalip].nil?))
        retval["host"] = Chef::Recipe::IPManagement.get_ip_for_net(retval["network"], nodeish)
      else
        retval["host"] = Chef::Recipe::IPManagement.get_ip_for_net(externalip, nodeish)
      end   
      retval["uri"] = "#{retval['scheme']}://#{retval['host']}:#{retval['port']}"
      retval["uri"] += retval["path"]
      retval
  ...
  def get_access_endpoint(role, server, service, externalip=nil)  
    
    Chef::Log.info("Get Access endpoint for role=#{role} server=#{server} service=#{service} externalip=#{externalip}")
    ...
    if result.length == 0
      ...
      get_bind_endpoint(server, service, result[0], externalip)
    end
  end
  ...
  #
  def get_realserver_endpoints(role, server, service, externalip=nil)
    query = "roles:#{role} AND chef_environment:#{node.chef_environment}"
    result, _, _ = Chef::Search::Query.new.search(:node, query)
    ...
    result.map { |nodeish| get_bind_endpoint(server, service, nodeish, externalip) }
  end
  ...
  def get_ips_for_role(role,network,node)
    retval=Chef::Recipe::IPManagement.get_ips_for_role(role,network,node)
    retval
  end
...
class Chef::Recipe::IPManagement
  ...
  def self.get_ip_for_net(network, node)
    Chef::Log.info("Get ip for net #{network} #{node}")
    if network == "all"
      return "0.0.0.0"
    end
    ...
    error = "Can't find address on network #{network}  #{node["osops_networks"][network]} #{net} for node"
    Chef::Log.error(error)
    raise error
  end
